...
path_weight=r"D:\project\code\JRSL_EHGNN\EHGNN\weights\\"
path_result= r"D:\project\code\JRSL_EHGNN\EHGNN\results\\"
...
for i in range(Train_num):
    ...
    net_input = np.concatenate((net_input, img_idx), axis=2)
    net_input = torch.from_numpy(net_input.astype(np.float32)).to(args.device)

    # model
    if mode_name=="EHGCN":
        net=EHGCN.EHGCN(height, width, bands+2, class_num, Q, A,alph=0.1).to(args.device)
    ...
    Draw_Classification_Map(classification_map, "results\\Image\\" +mode_name+"-"+ data_set_name + str(testOA)+"--")
    Draw_Classification_Map(masked_predicted_labels, "results\\Mask\\" + mode_name+"-"+data_set_name + str(testOA)+"--")
    ...
    if save_results==1:
        ...
        f = open(path_result + run_date +mode_name+"-"+data_set_name + '.txt', 'a+')
        str_results = '\n ======================' \
                    + '\nrun data = ' + run_date \
